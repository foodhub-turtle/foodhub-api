...
import { findAllByRestaurantId, findItemDetailsByRestaurantId, findOneMapping,findAllItemGroupMappingByItemMapId, findItemGroupByItemGroupId,findAllByCategoryId, findAllPopular, findBranchItemFilter, findByFilter, findAllItemVariants, findBoughtTogether } from "../../services/itemService.js";
// import { findByPk } from "../../services/restaurantService.js";
import catchAsync from "../../libs/catchAsync.js";
import { findAll, findAllFilter, findOrCreate, findByPk, update, deleteByPk, findByBranchId, findAllReview, findAllRating, findCustomerRating } from "../../services/branchService.js";
...
const { findItemMapByItemId, findItemMapByFilter } = require("../../services/itemMappingService.js");
...
const { item, branch, item_category, cuisine, restaurant_discount, review, ordered_restaurant, item_variant, item_group_mapping, item_mapping, item_group, ingredient_mapping, ingredient, restaurant_rating, setting } = Model;
...
export const getAllItemsByRestaurantId = catchAsync(async (req, res, next) => {
  ...
  for (let index = 0; index < result.category_id.length; index++) {
    ...
    var items = await findAllByCategoryId(item_mapping, {category_id: category.id, branch_id: result.id, status: 1});
    var itemsArray = [];
    for (let indexItem = 0; indexItem < items.length; indexItem++) {
      const elementItem = items[indexItem];
      let itemObj = await findItemDetailsByRestaurantId(item, {id: elementItem.item_id});
  ...
  var ratingObj = await findAllRating(ordered_restaurant, {branch_id: result.id});
  var reviews = await findAllReview(review, {branch_id: result.id});
  var favourites = await findAllRating(ordered_restaurant, {branch_id: result.id, is_favourite: 1});
...
export const branchItemFilter = catchAsync(async (req, res, next) => {
  ...
  if (filters.name) {
    andAttributes.push({name: {
      [Op.or]: [{[Op.iLike]: "%"+filters.name+"%"}, {[Op.iLike]: "%"+filters.name.toLowerCase()+"%"}]
    }});
  }
  andAttributes.push({branch_id: filters.branch});
  andAttributes.push({status: 1});
...
export const getItemByRestaurantId = catchAsync(async (req, res, next) => {
  let itemObj = await findItemDetailsByRestaurantId(item, {id: req.params.itemid});
  ...
  const itemMapping = await findItemMapByFilter(item_mapping, {item_id: itemObj.id, branch_id: req.params.restaurantid});
  ...
  if (itemMapping) {
    const itemMappingGroups = await findAllItemGroupMappingByItemMapId(item_group_mapping, {item_mapping_id: itemMapping.id, status: 1});
  ...
  if (itemMapping.is_hasVariants) {
    itemObj.priceVariants = await findAllItemVariants(item_variant, {item_id: itemMapping.item_id, branch_id: itemMapping.branch_id});
  }
  if (branchObj.is_showboughttogether == 1) {
    itemObj.boughttogether = await findBoughtTogether(item_mapping, {branch_id: branchObj.id});
  }
  return res.status(200).json({
    status: "success", 
    statusCode: 200,
