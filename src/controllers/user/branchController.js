...
export const getAllBranchsForCustomer = catchAsync(async (req, res, next) => {
  ...
  for (let index = 0; index < promotions.length; index++) {
    ...
    for (let rindex = 0; rindex < restaurant_promotions.length; rindex++) {
      ...
      var branchObj = await findByBranchId(branch, restaurant_promotion.branch_id);
      var origins = [req.query.lat+','+req.query.lng];
      var destinations = [branchObj.map_latitude+','+branchObj.map_longitude];
      if (branchObj.is_hasVoucher == 1) {
        var restaurantVoucher = await findVoucherBranch(voucher, {branch_id: restaurant_promotion.id});
        branchObj.voucher = restaurantVoucher;
      }
      let cuisines = [];

      for (let index = 0; index < branchObj.cuisine_id.length; index++) {
        let elementValue = branchObj.cuisine_id[index];
        let cuisineObj = await cuisineService.findByPk(cuisine, elementValue);
        cuisines.push(_.omit(cuisineObj));
      }
      var ratingObj = await findAllRating(ordered_restaurant, {branch_id: restaurant_promotion.id});
      var reviews = await findAllReview(review, {branch_id: restaurant_promotion.id});
      var favourites = await findAllRating(ordered_restaurant, {branch_id: restaurant_promotion.id, is_favourite: 1});
      branchObj.cuisine = (cuisines.length > 0) ? cuisines[0] : [];
      ...
      if (branchObj.is_hasDiscount) {
        var restaurantDiscount = await findByFilter(restaurant_discount, {branch_id: restaurant_promotion.id});
        branchObj.discount = restaurantDiscount;
      }
      promotionBranches.push(branchObj);
    }
  ...
  currentVouchers = _.filter(currentVouchers, (voucher) => {
    let beforeTime = moment(voucher.start_time, 'hh:mm:ss'), afterTime = moment(voucher.end_time, 'hh:mm:ss');
